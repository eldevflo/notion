...
const port = 8000;
const sqlite = app.use((0, cors_1.default)());
...
let db = new sqlite3_1.Database('credentials.db', (error) => {
    if (error) {
...
app.listen(port, () => {
    ...
    //   },
    // )
    db.all(`INSERT INTO credentials(username , password, email) VALUES ('elena', '1234','f.zamanipour@gmail.com')`, (error, rows) => {
        if (error) {
            throw error;
        }
    });
